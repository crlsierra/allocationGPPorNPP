...
\usepackage{graphicx}
%\graphicspath{{~/Repos/cmip6stop/esm-hist/Figures/}{~/MPIBGC-TEE/bgc_md2/notebooks/Emanuel/xiGPPUpp/Figures/}}
\usepackage{bm}
\usepackage{booktabs}
...
%\subsection{\citet{Amthor2000}'s respiration paradigms}
...
%A very common approached implemented in ecosystem models is to partition the \emph{net} carbon gained through photosynthesis. This net gain is often calculated by subtracting $Ra$ from GPP. 
%Two types of autotrophic respiration can be found in models: growth and maintenance respiration. Growth respiration represents the cost of growing organs, which is often expressed as a percentage of the C influx (in most cases GPP) that cannot be used to build new tissues and is independent from the amount of C stored in the vegetation compartments. Maintenance respiration, by contrast, is the metabolic cost of maintaining the existing tissue, and it is usually computed as a proportion of the current carbon stock, but there is inconsistency on how this is actually implemented in many models. 
%These two representations have different implications for carbon age calculations. The carbon respired for growth is subtracted from the newly acquired C via photosynthesis; whereas the maintenance respiration comes from carbon stored in the vegetation tissues.
...
%As long as we conceive the tree as a continuously acting entity, we tend to describe and model it as a system of ordinary differential equations.
%The model implementation on the other hand is usually based on discrete time steps, which makes the system be described better by a system of ordinary difference equations.
%The big difference here is that newly captured carbon that can be used for maintenance respiration in a continuous-time thinking, in a discrete-time implemented model 
...
%This becomes obvious if we think about a yearly time step.
...
%Maintenance respiration should by definition come from the tree, growth respiration from newly captured carbon.
%This role is reversed here simply by the discretized implementation.
...
%The former means that carbon sits there for a year before it can be respired for growth, the latter means that carbon that is respired for maintenance does not even enter the tree.
%That maintenance respiration carbon does not even enter the tree leads to conceptual problems in later stages of the physiological understanding.
%When we think about the path of a carbon atom through the tree, a carbon atom that has never entered the tree is hard to tackle.
%By and large it is of course possible to count all the carbon atoms that never entered the tree by GPP-NPP and make some adaptations in the process of computing transit times and carbon ages within the tree.
%But all in all the carbon never entering the tree is missing in the tree and biasing the age structure by its absence.
%Any corrections to that are kind of guess work and obscure the real understanding.
...
%To implement the maintenance respiration in a fair sense with respect to growth respiration, in a discrete time model with a yearly time step captured carbon has to stay in the transient pool and sit around there doing nothing for one year, because only in the next year it can be used for allocation and respiration.
%In the meantime this transient pool is also refilled with carbon from nonstructural carbohydrate storages that became available because leaves or roots died.
%Thus, the transient pool in the next year has a different age structure than the year before, and a carbon atom used for maintenance does not necessarily have age zero when it leaves the tree, it might actually have been in the tree for quite a while, serving other purposes.

%In the conceptual thinking as a continuous-time process this discrepancy does not occur, but it becomes apparent when we implement the continuous-time concepts as discrete-time models.

%\red{My personal suggestion to solve this issue is to get rid of the transient pool in the first place, which requires a conceptual rethinking. But I did not go that far, maybe unfortunately.}
...
%In addition to this equilibrium analysis, we ran the GPP-based version of the model in a transient simulation with increases in GPP and decomposition as in traditional historical simulations of carbon response to anthropogenic perturbations. In these simulations, the transit time of carbon decreases over time as GPP increases and decomposition rates increase. A cross correlation analysis shows that autotrophic, heterotrophic, and ecosystem respiration are correlated with GPP with a lag between 6 to 10 yr, which can be explained by the transit time of carbon across vegetation compartments. A similar cross correlation analysis of the output from models participating in CMIP6 showed maximum correlations with no time lag; i.e. respiration in the global models respond to external forcing at the same timescale as GPP, possibly because in all of them the transit time of carbon in vegetation compartments is close to zero. 
...
%This makes evident that the mean age of the respired carbon by vegetation may be on average older than one year for most ecosystems on earth. Considering this mean age a good representation of the mean transit time of the carbon in the vegetation system, the offset between the mean transit time estimations from GPP based model and NPP based model tells us the importance to GPP allocation principles to study the time scales at which carbon is cycle in the ecosystem. 

%Additionally, the ecosystem mean transit time has also contributions from decomposers and soil respiration. The age of the respired carbon from these pools may vary from 16 to 1 years old depending on the ecosystem \citep{Trumbore:2000vv}. Nevertheless, despite these fluxes are mostly compound by young carbon, the mean ages are larger than one. 
...
\begin{figure}[htbp]
   ...
   \caption{Age of C in respired CO$_2$ from roots and stems for different tree species from temperate and tropical forests obtained from radiocarbon measurements. Data for roots include both fine and coarse roots, and data for stems is split between chamber based measurements and incubations of tree cores. Numbers on top of the boxes represent the number of observations available to draw the boxplots. Values below the horizontal dashed line represent measurements of carbon younger than 1 yr.} %Additional details are presented in Supplementary Table 1.}
...
%\subsection{Use of radiocarbon observations for model-data integration}
...
%\subsection{Obtaining the transit time distribution of carbon in terrestrial ecosystems}
...
%\subsection{Atmosphere-biosphere isotopic exchange}
...
%The carbon inventory of an ecosystem has accumulated over several growing seasons and thus is not only composed of recently-fixed carbon. The process of respiration in cells is based on the oxidation of organic molecules, which are part of the carbon inventory of an ecosystem from where the respiration flux must be drawn. 
...
\bibliography{allocationNPP}

%\begin{appendix}
%\section{Cross correlation analysis}
%To better understand the dynamic response of respiration in Earth system models, we performed a cross correlation analysis between respiration and gross primary production. Our aim was to identify a time lag at which correlation between these variables is higher. Our hypothesis is that in GPP-based models, the transit time of carbon produces a time lag in the response of respiration to external forcing. This hypothesis was tested first with the GPP-based version of the Emanuel model to identify such a time lag. Subsequently, we tested this hypothesis with all available models from the CMIP6 archive. 
%
%
%
%\subsection{Transient simulation with Emanuel's model}
%
%\begin{figure}[htbp]
%   \centering
%   \includegraphics{corRaGPP.pdf} % requires the graphicx package
%   \caption{Cross correlation between autotrophic respiration and gross primary production in the GPP-based version of the Emanuel model. }
%   \label{fig:corRaGPP}
%\end{figure}
%
%\subsection{CMIP6 models}
%
%\begin{figure}[htbp]
%   \centering
%   \includegraphics[scale=1]{RaGPP.pdf} 
%   \caption{Numerical output of gross primary production and autotrophic respiration from Earth system models participating in CMIP6. The original monthly values reported by CMIP6 were aggregated by cell size in panels a and b. Annual aggregation is presented in panels c and d. }
%   \label{fig:annualGPP}
%\end{figure}
%
%\begin{figure}[htbp]
%   \centering
%   \includegraphics{corRaGPPmonth.pdf} % requires the graphicx package
%   \caption{Cross correlation between autotrophic respiration and gross primary production in Earth system models participating in CMIP6. Monthly model output was aggregated for all grid cells.}
%   \label{fig:corRaGPPmonth}
%\end{figure}
%
%\begin{figure}[htbp]
%   \centering
%   \includegraphics{corRaGPPyr.pdf} % requires the graphicx package
%   \caption{Cross correlation between autotrophic respiration and gross primary production in Earth system models participating in CMIP6. Model output was aggregated for all grid cells for each year.}
%   \label{fig:corRaGPPyr}
%\end{figure}
%
%
%\end{appendix}
